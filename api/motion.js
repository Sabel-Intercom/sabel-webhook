...
export default async function handler(req, res) {
  ...
  try {
      ...
      // Motion's rate limits).
      for (let page = 0; page < 5; page++) {
              const url = MOTION_API + (cursor ? '?cursor=' + encodeURIComponent(cursor) : '');
      ...
      res.setHeader('Cache-Control', 's-maxage=120');
        return res.status(200).json({ ok: true, since: since, count: out.length, tasks: out });
  } catch (err) {
